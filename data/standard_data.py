...
# from torchvision import transforms as T  # 避免和 torch.nn.transforms 混淆
...
#         path = self.path_list[idx]
#         filename = op.splitext(op.basename(path))[0]
#         img = np.load(path).transpose(1, 2, 0)  # (C, H, W) → (H, W, C)

#         # Normalize 用来统一图像分布
#         if self.train and self.aug:
#             transform = T.Compose([
#                 T.ToTensor(),
#                 T.RandomHorizontalFlip(self.aug_prob),
#                 T.RandomVerticalFlip(self.aug_prob),
#                 T.RandomRotation(10),
#                 T.RandomCrop(128),
...
#         img_tensor = transform(img)  # 输入为 HWC ndarray，ToTensor 会转成 CxHxW Tensor
